...
TEST(RSA, NoPadding)
{
	...
	ctext = Rsa::encrypt(ptext, RSA_PUBLIC_KEY.get(), Rsa::PublicKey, Rsa::NoPadding);
...
TEST(RSA, OAEPwithSHA1andMGF1Padding)
{
	...
	ptext = Rsa::decrypt(ctext, RSA_PRIVATE_KEY.get(), Rsa::PrivateKey, Rsa::OAEPwithSHA1andMGF1Padding);
...
TEST(RSA, PKCS1v15Padding)
{
	...
	ptext = Rsa::decrypt(ctext, RSA_PRIVATE_KEY.get(), Rsa::PrivateKey, Rsa::PKCS1Padding);
	EXPECT_EQ(Base64::encode(ptext), "GcRqZtvZqfz0nww=");
