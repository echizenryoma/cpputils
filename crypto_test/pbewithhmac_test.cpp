...
TEST(PBEwithHmac, HmacSHA1)
{
	...
	c = 1;
	dkLen = 20;
	DK = PBEwithHmac::derived(vector<byte>(P.begin(), P.end()), vector<byte>(S.begin(), S.end()), c, PBEwithHmac::HmacSHA1, dkLen);
...
//	EXPECT_EQ(Hex::encode(DK), "EEFE3D61CD4DA4E4E9945B3D6BA2158C2634E984");
	...
	c = 4096;
	dkLen = 16;
	DK = PBEwithHmac::derived(vector<byte>(&P_cstr[0], &P_cstr[0] + P_cstr_size), vector<byte>(&S_cstr[0], &S_cstr[0] + S_cstr_size), c, PBEwithHmac::HmacSHA1, dkLen);
